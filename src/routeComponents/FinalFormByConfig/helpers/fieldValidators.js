...
export const pipeValidatorsAndGetHead = (...validators) => (...valueArgs) => {
  return R.pipe(
...
export const pipeValidatorsAndGetAll = (...validators) => (...valueArgs) => {
  return R.pipe(R.converge(R.identity, validators), RA.compact)(...valueArgs)
}
...
export const isNumber = R.pipe(
  Number,
  R.ifElse(R.allPass([RA.isNumber, RA.isNotNaN]), R.always(undefined), () => ({
...
export const isPositiveNumber = R.pipe(
  Number,
  R.ifElse(RA.isPositive, R.always(undefined), () => ({
...
export const isNonNegative = R.pipe(
  Number,
  R.ifElse(RA.isNonNegative, R.always(undefined), () => ({
    validatedBy: isNonNegative,
    msg: t('error.shouldBeANonNegativeNumber'),
  })),
)

export const isGreaterThan = smaller =>
  R.pipe(
    Number,
    R.ifElse(R.gt(R.__, smaller), R.always(undefined), () => ({
      validatedBy: isGreaterThan,
      msg: t('error.shouldBeGT', { n: smaller }),
    })),
  )

export const isGreaterThanOrEqualTo = smaller =>
  R.pipe(
    Number,
    R.ifElse(R.gte(R.__, smaller), R.always(undefined), () => ({
      validatedBy: isGreaterThanOrEqualTo,
      msg: t('error.shouldBeGTE', { n: smaller }),
    })),
  )

export const isLessThan = larger =>
  R.pipe(
    Number,
    R.ifElse(R.gt(larger), R.always(undefined), () => ({
      validatedBy: isLessThan,
      msg: t('error.shouldBeLT', { n: larger }),
    })),
  )

export const isLessThanOrEqualTo = larger =>
  R.pipe(
    Number,
    R.ifElse(R.gte(larger), R.always(undefined), () => ({
      validatedBy: isLessThanOrEqualTo,
      msg: t('error.shouldBeLTE', { n: larger }),
    })),
  )

export const isMoreThanNChars = n =>
  R.ifElse(
    x => `${x}`.length >= n,
    R.always(undefined),
    () => ({
...
export const isLessThanNChars = n =>
  R.ifElse(
    x => `${x}`.length <= n,
    R.always(undefined),
    () => ({
