...
class App extends Component {
  Sub = {
    Tabs: ({ match, history }) => {
      ...
      return (
        <div>
          <Tabs
            value={R.ifElse(R.equals('/'), R.always(null), x => {
              return R.find(
                R.where({
                  path: R.contains(x),
                }),
          ...
          >
            {Object.values(paths).map(x => (
              <Tab key={x.name} label={x.displayText} value={x} />
            ))}
          </Tabs>
        </div>
      )
  ...
  render() {
    return (
      <BrowserRouter>
        <div className="App">
          <Route path="/" component={this.Sub.Tabs} />

          <Route
            path={paths.downshiftExamples.path}
            component={DownShiftExamples}
          />

          <Route path={paths.DnDExamples.path} component={DnDExamples} />

          <Route
            path={paths.FinalFormByConfigs.path}
            component={FinalFormByConfig}
          />

          <Route path={paths.HookExamples.path} component={HookExamples} />

          <Route path={paths.MUIReactTable.path} component={MUIReactTables} />
        </div>
      </BrowserRouter>
    )
  }
